...
function renderScope(
  ...
  const instructionMarkup =
    scopeInstructions.length === 0
      ...
      : `<ol class="instruction-list">${scopeInstructions.map(renderInstruction).join("")}</ol>`;

  return `<details class="scope"${index === 0 ? " open" : ""}>
  <summary>
    <span class="scope-index">${String(index + 1).padStart(2, "0")}</span>
    <span class="scope-heading"><strong class="untrusted">${escapeHtml(scope.path)}</strong><small>precedence ${scope.precedence} · ~${scope.tokenEstimate.total.toLocaleString("en-US")} tokens</small></span>
    <span class="scope-count">${scopeInstructions.length}</span>
  </summary>
...
.brand{display:inline-flex;align-items:center;gap:.55rem;margin:0 0 2.5rem;font-size:.77rem;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:var(--accent)}
...
.metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:1px;max-width:1120px;margin:2.5rem auto 0;background:var(--line);border:1px solid var(--line);border-radius:1rem;overflow:hidden}
.metric{padding:1.2rem;background:var(--panel)}
.metric strong{display:block;font-family:Georgia,"Times New Roman",serif;font-size:2rem;font-weight:500}.metric span{color:var(--muted);font-size:.78rem;text-transform:uppercase;letter-spacing:.09em}
...
.section-head{display:grid;grid-template-columns:1fr 1fr;gap:2rem;align-items:end;margin-bottom:1.5rem}.section-head h2{margin:0;font:500 clamp(2rem,5vw,4rem)/1 Georgia,"Times New Roman",serif;letter-spacing:-.04em}.section-head p{max-width:480px;margin:0;color:var(--muted);line-height:1.6}
.scope{border-top:1px solid var(--line)}.scope:last-child{border-bottom:1px solid var(--line)}
.scope summary{display:grid;grid-template-columns:3rem 1fr auto;gap:1rem;align-items:center;padding:1.25rem .25rem;cursor:pointer;list-style:none}.scope summary::-webkit-details-marker{display:none}.scope summary:focus-visible{outline:3px solid #7bb7a4;outline-offset:4px;border-radius:.35rem}.scope-index{font:700 .75rem/1 ui-monospace,monospace;color:var(--accent)}.scope-heading{display:grid;gap:.25rem}.scope-heading strong{font-size:1rem}.scope-heading small{color:var(--muted)}.scope-count{display:grid;place-items:center;width:2.25rem;height:2.25rem;border-radius:50%;background:var(--accent-soft);color:var(--accent);font-weight:800}
...
.diagnostic-list{display:grid;gap:.75rem;margin:0;padding:0;list-style:none}.diagnostic{padding:1.15rem 1.25rem;border:1px solid var(--line);border-left:4px solid var(--info);border-radius:.65rem;background:var(--panel)}.diagnostic-error{border-left-color:var(--error)}.diagnostic-warning{border-left-color:var(--warn)}.diagnostic>div{display:flex;gap:.75rem;align-items:center}.diagnostic p{margin:.65rem 0 0;line-height:1.5}.severity{font-size:.68rem;font-weight:900;letter-spacing:.1em}.diagnostic code{color:var(--muted);font-size:.76rem}.source-list{margin:.65rem 0 0;padding-left:1.15rem;color:var(--muted);font:500 .72rem/1.6 ui-monospace,monospace}
.footer{margin-top:5rem;padding-top:1.5rem;border-top:1px solid var(--line);color:var(--muted);font-size:.78rem}
@media(max-width:700px){.metrics{grid-template-columns:1fr 1fr}.section-head{grid-template-columns:1fr}.scope summary{grid-template-columns:2rem 1fr auto;gap:.55rem}.scope-body{padding-left:2.5rem}main{width:min(100% - 1.5rem,1120px)}}
...
@media print{*{color:#000!important;background:#fff!important;box-shadow:none!important}.hero{padding:1cm 0}.metrics{break-inside:avoid}main{width:100%;padding:1cm 0}.scope{break-inside:avoid}.scope-body{display:block}.footer{margin-top:1cm}}
`;
...
export function renderHtml(report: ScopeglassReportV1): string {
  ...
  const diagnosticMarkup =
    report.diagnostics.length === 0
      ...
      : `<ul class="diagnostic-list">${report.diagnostics.map(renderDiagnostic).join("")}</ul>`;
...
<body>
  <header class="hero">
    <p class="brand">Scopeglass · ruleset v${report.rulesetVersion}</p>
    ...
    <div class="metrics" aria-label="Analysis summary">
      <div class="metric"><strong>${report.summary.scopeCount}</strong><span>Scopes</span></div>
      <div class="metric"><strong>${report.summary.instructionCount}</strong><span>Instructions</span></div>
      <div class="metric"><strong>${report.tokenEstimate.total.toLocaleString("en-US")}</strong><span>Est. tokens</span></div>
      <div class="metric"><strong>${report.diagnostics.length}</strong><span>Diagnostics</span></div>
    </div>
  </header>
  <main>
